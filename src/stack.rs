...
impl <T> Stack<T> {
    fn new() -> Stack<T> { Stack{vec: Vec::new(),} }

    // Adds an element at the top of the stack
    fn push(&mut self, elem: T) { self.vec.push(elem); }

    // Removes and returns the element at the top of the stack
    fn pop(&mut self) -> Option<T> { self.vec.pop() }

    // Returns a reference of the element at the top of the stack
    fn peek<'r>(&'r self) -> Option<&'r T> { self.vec.last() }

    // Returns true if the stack is empty
    fn empty(&self) -> bool { self.vec.len() == 0 }
}
...
fn main() {
    ...
    } else { println!("The stack is not empty.") }
}
