...
fn compress(original_str: &str) -> Vec<int> {
    ...
    for i in range(0, dict_size) { dictionary.insert(vec!(i as u8), i); }
    ...
    if w.len() > 0 { result.push(*dictionary.get(&w)); }
...
fn decompress(compressed: &Vec<int>) -> ~str {
    ...
    for i in range(0, dict_size) { dictionary.insert(i, vec!(i as u8)); }
...
fn test_example() {
    let original = "TOBEORNOTTOBEORTOBEORNOT";
    assert!(compress ( original ) . as_slice ( ) ==
