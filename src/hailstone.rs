...
struct Hailstone {
    current: uint,
     // Accessible online only to itself.
    start: uint, // Accessible to everything.
}

// Define a constructor for the struct.
impl Hailstone {
    fn new(n: uint) -> Hailstone { Hailstone{current: n, start: n,} }
}
...
fn main() {
    ...
    println!("Testing: {}, Length: {}, Values: {}...{}" , two_seven . get
             ( 0 ) , two_seven . len ( ) , two_seven . slice ( 0 , 4 ) .
             to_str ( ) , two_seven . slice
             ( two_seven . len ( ) - 4 , two_seven . len ( ) ) . to_str ( ));
...
fn test_27() {
    ...
    assert!(seq . slice ( seq . len ( ) - 4 , seq . len ( ) ) ==
            [ 8 , 4 , 2 , 1 ]);
}
